        ...
        try:
            ...
            try:
                ...
                with open(os.path.join(config_path, "allowupdatepip.txt"), "r") as f:
                    lines = f.readlines()
                    if len(lines) >= 2:
                        allow_check = lines[0].strip().lower() == "true"
                        last_checked_version = lines[1].strip()
                        
                        # 如果不允许检查且版本没变，则跳过
                        if not allow_check and current_version == last_check_version:
                            check_needed = False
            except:
                pass
            
            # 如果需要检查或版本已变更，则启动检查线程
            if check_needed or current_version != last_check_version:
                last_check_version = current_version
                t = threading.Thread(target=thread, daemon=True)
                t.start()
                
                # 设置定时器在主线程中更新UI
                root.after(500, update_pip_ui)
            
            time.sleep(2.0)  # 适当增加检查间隔
        except Exception as e:
            logger.error(f"保存路径或检查pip版本出错: {e}")
...
def cancel_download():
    """取消下载"""
    global is_downloading, is_paused, futures, executor
...
def show_pip_version():
    """显示pip版本"""
    global pip_upgrade_button, pip_retry_button, pip_version_check_status
    
    # 在主线程中设置定时检查
    def check_status_periodically():
        # 在主线程中检查状态并更新UI
        if hasattr(globals(), 'pip_version_check_status') and pip_version_check_status is not None:
            update_pip_ui()
        # 继续定期检查
        root.after(500, check_status_periodically)
    ...
    # 启动定期检查状态的定时器（在主线程中）
    root.after(100, check_status_periodically)
