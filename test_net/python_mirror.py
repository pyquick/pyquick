...
提供测试Python官方和第三方镜像下载速度的图形界面，
帮助用户选择最佳的Python安装包下载源。
"""
...
# 设置日志
logger = logging.getLogger("test_net.python_mirror")

# 默认Python镜像列表
DEFAULT_PYTHON_MIRRORS = {
    "Python官方": "https://www.python.org/ftp/python/",
    "华为云": "https://mirrors.huaweicloud.com/python/",
    "淘宝": "https://npm.taobao.org/mirrors/python/",
    "TUNA清华": "https://mirrors.tuna.tsinghua.edu.cn/python/",
    "网易": "https://mirrors.163.com/python/",
    "USTC中科大": "https://mirrors.ustc.edu.cn/python/"
}

class PythonMirrorTester:
    """Python镜像测试工具类，提供图形界面测试不同Python下载镜像的速度"""
    ...
    def __init__(self, root=None, callback=None):
        ...
        """
        self.mirrors = DEFAULT_PYTHON_MIRRORS.copy()
    ...
    def load_custom_mirrors(self):
        """加载自定义镜像"""
        try:
            config_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "settings", "python_mirrors.json")
            if os.path.exists(config_path):
                with open(config_path, "r", encoding="utf-8") as f:
                    ...
                    if isinstance(custom_mirrors, dict):
                        ...
                        logger.info(f"已加载{len(custom_mirrors)}个自定义Python镜像")
        except Exception as e:
            logger.error(f"加载自定义Python镜像失败: {str(e)}")
    ...
    def save_custom_mirrors(self):
        ...
        try:
            ...
            custom_mirrors = {}
            for name, url in self.mirrors.items():
                if name not in DEFAULT_PYTHON_MIRRORS or url != DEFAULT_PYTHON_MIRRORS[name]:
            ...
            # 保存到文件
            config_path = os.path.join(settings_dir, "python_mirrors.json")
            with open(config_path, "w", encoding="utf-8") as f:
                json.dump(custom_mirrors, f, ensure_ascii=False, indent=4)
                
            logger.info(f"已保存{len(custom_mirrors)}个自定义Python镜像")
        except Exception as e:
            logger.error(f"保存自定义Python镜像失败: {str(e)}")
    ...
    def delete_mirror(self):
        ...
        # 检查是否为默认镜像
        if name in DEFAULT_PYTHON_MIRRORS and self.mirrors[name] == DEFAULT_PYTHON_MIRRORS[name]:
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("Python镜像测试")
    
    def on_mirror_selected(name, url):
        print(f"选择的镜像: {name}, URL: {url}")
    
    app = PythonMirrorTester(root, callback=on_mirror_selected)
