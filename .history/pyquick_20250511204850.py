...
def signal_handler(sig, frame):
    app_logger.info("检测到CTRL+C,正在退出程序...")
    # 执行清理操作
    on_closing()
    # 退出程序
    app_logger.info("程序已退出。")
    sys.exit(0)
...
settings_manager.auto_save_system_python()
app_logger.info("系统Python版本信息已自动扫描并更新。")
try:
    # SettingsManager from settings.save is used here
    # config_path should be initialized and finalized by now
    save_settings_manager_instance = SettingsManager(config_path=config_path)
    save_settings_manager_instance.auto_save_system_python()
    app_logger.info("系统Python版本信息已自动扫描并更新。")
except Exception as e:
    error_logger.error(f"自动扫描并更新系统Python版本信息时出错: {str(e)}")
    ...
    try:
        ...
        for thread in threading.enumerate():
            if thread != threading.current_thread():
                app_logger.info(f"正在终止线程: {thread.name}")
                try:
                    ...
                    error_logger.error(f"终止线程{thread.name}失败: {e}")
