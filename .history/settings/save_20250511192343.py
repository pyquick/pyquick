...
class SettingsManager:
    ...
    def scan_system_python_installations(self):
        """
        扫描系统中安装的Python版本
    ...
    def update_python_installations(self):
        """
    ...
    def auto_save_system_python(self):
        """
    ...
    def _merge_user_settings(self, user_settings_model, merge_base, key_path = ""):
        """
        递归合并用户设置模型到基础字典中，处理嵌套字典结构
        
        该方法递归地将用户设置模型(user_settings_model)合并到基础字典(merge_base)中，
        同时处理key_path的点分格式路径，用于日志记录和错误追踪。
        
        Args:
            user_settings_model: 要合并的用户设置字典，包含新的设置值
            merge_base: 目标字典，通常是默认设置或父级设置字典
            key_path: 当前递归路径，用于日志和错误追踪，格式为"parent.child"
            
        Returns:
            Tuple[Dict[str, Any], bool]: 
                第一个元素是合并后的设置字典
                第二个元素表示是否成功合并(True/False)
        
        Raises:
            RecursionError: 当递归深度超过20层时记录警告
        ...
        try:
            ...
            for key, value in user_settings_model.items():
                ...
                if isinstance(value, dict) and key in merge_base and isinstance(merge_base[key], dict):
                    # 递归合并嵌套字典
                    merged[key], success = self._merge_user_settings(value, merge_base[key], current_path)
                    if not success:
                        return merged, False
                # 处理基础字典中有值的情况
                elif key in merge_base and merge_base[key] is not None:
                    # 保留基础字典中的值
                    merged[key] = merge_base[key]
                else:
                    # 直接使用用户设置的值
                    merged[key] = value
    ...
    def _merge_settings(self, default, user):
        ...
        Returns:
            合并后的设置
        """
        merged, _ = self._merge_user_settings(user, default, "")
        return merged
    ...
    def _schedule_auto_save(self, delay = 5):
        """
        计划自动保存
