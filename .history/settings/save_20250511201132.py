...
class SettingsManager:
    ...
    def _merge_user_settings(self, user_settings_model: Dict[str, Any], merge_base: Dict[str, Any], key_path: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            # 特殊处理python_versions.installations路径，确保完全替换而不是追加
            if key_path == "python_versions.installations":
                logger.debug(f"特殊处理python_versions.installations路径，完全替换而不是追加")
            ...
            for key, value in user_settings_model.items():
                ...
                # 特殊处理installations键，确保完全替换而不是追加
                if current_path == "python_versions.installations" or key == "installations" and key_path == "python_versions":
                    logger.debug(f"特殊处理installations键，完全替换而不是追加: {current_path}")
                    merged[key] = value
                    continue
                ...
                if isinstance(value, dict) and key in merged and isinstance(merged[key], dict):
                    # 递归合并嵌套字典
                    merged[key] = self._merge_user_settings(value, merged[key], current_path)
                else:
                    # 使用用户设置的值覆盖基础设置
                    merged[key] = value
